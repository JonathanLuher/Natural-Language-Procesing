...
# ================================
# CELDA 3: CÓDIGO PRINCIPAL - SOLO ROBERTA
# ================================
...
# ================================
# CELDA 5: ENTRENAR ROBERTA
# ================================

print("🚀 ENTRENANDO ROBERTA...")

# Configuración específica para RoBERTa
...
# Configurar argumentos de entrenamiento (optimizados para RoBERTa en Colab)
training_args = TrainingArguments(
    output_dir='/content/roberta_model',
    num_train_epochs=2,  # Reducido para Colab
    per_device_train_batch_size=8,  # Batch pequeño para modelo grande
    per_device_eval_batch_size=8,
    learning_rate=2e-5,  # LR específico para RoBERTa
    weight_decay=0.01,
    logging_steps=50,
    eval_strategy='steps',
    eval_steps=100,
...
if 'TA1C_dataset_detection_dev.csv' in [f for f in os.listdir('.') if f.endswith('.csv')]:
    ...
    try:
        ...
        results_df.to_csv('detection_roberta.csv', index=False)
        print("✅ Archivo 'detection_roberta.csv' generado!")

        # Descargar archivo
        files.download('detection_roberta.csv')
